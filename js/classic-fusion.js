...
(function () {
  ...
  function open() {
    ...
    function validation(gears) {
      ...
      if (materials.some((gear) => gear.quality >= 3)) return '卓越装备已是最高品质';
    ...
    function render(focus) {
      ...
      selected = selected.filter((key) => gears.some((gear) => gear.key === key && !gear.used && gear.quality < 3));
      const first = gears.find((gear) => gear.key === selected[0]);
      // The original dictionary installs its own window.Map implementation.
      const groups = Object.create(null);
      gears.forEach((gear) => {
        if (!gear.used && gear.quality < 3) groups[gear.id] = (groups[gear.id] || 0) + 1;
      });
      const inventory = gears.slice().sort((a, b) =>
        Number(a.used || a.quality >= 3) - Number(b.used || b.quality >= 3) ||
      ...
      const cards = inventory.slice(pageIndex * PAGE_SIZE, (pageIndex + 1) * PAGE_SIZE).map((gear) => {
        ...
        const unavailable = gear.used ? '已穿戴' : gear.quality >= 3 ? '最高品质' : first && gear.id !== first.id ? '需要相同装备' : selected.length === 3 && !chosen ? '材料已放满' : '';
      ...
      board.innerHTML = '<header class="fusion-heading"><h2>装备融合</h2><span class="fusion-wallet">' + classic.spr('resource_1', 18) + '<span>金松果 <b>' + State.state().goldPoint + '</b></span></span></header>' +
        ...
        '<p class="fusion-rules">3 件相同装备 + 50 金松果，获得 1 件随机高一级品质装备；材料会被消耗。</p>' +
      ...
      board.querySelectorAll('[data-fusion-gear]').forEach((button) => {
        button.onclick = () => {
          ...
          if (!gear || gear.used || gear.quality >= 3) return;
