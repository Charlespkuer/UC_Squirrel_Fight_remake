...
同名文件」三种候选查找，比较大小与 SHA-256。脚本：`references/tools/apk-audit/compare.cjs`，
明细：`references/tools/apk-audit/report.md`。
...
# 5) 在压缩源码里检索并解码文案（免解压）
node references/tools/apk-audit/search.cjs "fightingTimes==30" --before 200 --after 400 --context
node references/tools/apk-audit/search.cjs --read js/ssdz-pkg2.js --from 54600 --len 3200 --decode
```
