...
只读分析，未修改 APK。取数工具：`references/tools/apk-audit/apk-zip.cjs`（免解压直接读 APK）、
`references/tools/apk-audit/view-layout.cjs`（导出某个界面的原始坐标）。
...
客户端那句 `19+t*3` 与写死的「金松果25个」；这一点与本项目 `references/tools/balance-audit.md`
...
```
$ node references/tools/apk-audit/view-layout.cjs Shop
...
node references/tools/apk-audit/apk-zip.cjs list
node references/tools/apk-audit/apk-zip.cjs read js/player.js

# 列出 37 个原始界面 + 元素数
node references/tools/apk-audit/view-layout.cjs --list

# 导出单个界面的坐标表 / JSON / 用到的素材 / 调用的服务端接口
node references/tools/apk-audit/view-layout.cjs Mission
node references/tools/apk-audit/view-layout.cjs MyWeapon --json

# 在压缩代码里定位一段逻辑
node references/tools/apk-audit/extract-window.cjs "fightingTimes==30" 700 700

# 与项目资源逐字节核对
node references/tools/apk-audit/compare.cjs
```
...
3. **战斗结算公式仍需离线设计**，但要接受它无法从客户端"找回"：客户端只播不下算。
   本项目 `references/tools/research-battle-rules.md` 的记录方式（明确标注哪些是离线取值）是正确做法。
