...
try { graphics = require('@napi-rs/canvas'); }
  ...
  // 可选依赖：常见的位置都找一遍（Codex 运行时、全局 node_modules），找不到就跳过这一项
  const candidates = [
    process.env.NAPI_CANVAS_PATH,
    process.env.HOME && path.join(process.env.HOME, '.cache/codex-runtimes/codex-primary-runtime/dependencies/node/node_modules/@napi-rs/canvas'),
    process.env.USERPROFILE && path.join(process.env.USERPROFILE, '.cache/codex-runtimes/codex-primary-runtime/dependencies/node/node_modules/@napi-rs/canvas'),
    '/usr/local/lib/node_modules/@napi-rs/canvas',
  ].filter(Boolean);
  for (const dir of candidates) { try { graphics = require(dir); break; } catch (e) { /* 继续找 */ } }
}
if (!graphics) {
  console.log('SKIP tools/test-battle.js：需要可选依赖 @napi-rs/canvas');
  console.log('     装上就能跑：npm install --no-save --package-lock=false @napi-rs/canvas');
  process.exit(0);
}
let time = 0, seq = 0;
