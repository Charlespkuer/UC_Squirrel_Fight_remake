 ...
 *
 *   node tools/clean-git-history.cjs            # 演练：只报告会删什么、能省多少
 *   node tools/clean-git-history.cjs --apply    # 真的重写历史（需要重新 clone 或 force push）
 *
 * 为什么需要：`.gitignore` 早就把 `references/*`（原 APK、参考截图）、`fonts/` 排除在外了，但它们**早期已经被提交过**，打包进 .git 之后
...
// （原 APK、参考截图等），fonts/ 已弃用，electron/ 与根目录那几个是历史遗留文件。
// 注意：**不要**把 `tools/**` 放进来——工具现在就在仓库根，是活文件（历史里那批旧位置的
// 副本在 2026-09-25 那次重写里已经清掉了）。
const PURGE = [
  ':(glob)references/**',
  ':(glob)fonts/**',
  ':(glob)electron/**',
...
const affected = trackedBefore.filter((p) =>
  /^(fonts|electron)\//.test(p) || /^references\//.test(p) ||
...
function kept(cwd) {
  ...
  for (const line of lines.split('\n').filter(Boolean)) {
    ...
    if (/^(fonts|electron)\//.test(name) || /^references\//.test(name)) continue;
