...
> 取证方式：`tools/apk-audit/apk-zip.cjs`（零依赖直读 APK 内层 zip）。
...
```bash
node tools/apk-audit/apk-zip.cjs list                 # 列出内层 456 个文件
node tools/apk-audit/apk-zip.cjs read js/GameDict.js  # 直接看某个文件
node tools/apk-audit/compare.cjs                     # 与项目资源逐字节核对
node tools/apk-audit/view-layout.cjs --list          # 37 个界面的原始坐标
```
