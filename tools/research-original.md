...
原客户端能确认整轮经验与金松果奖励总量，但没有原服务器的掉落概率和三战奖励拆分。离线版据此保留整轮的**经验总量**，在三战全部通关后一次发放；关卡碎片改为每次击败对手都可能掉落的本地概率实现。这部分是明确的离线实现，不能称为找回了原服务器结算规则。关卡回归 `tools/test-stages.cjs`、掉落回归 `tools/test-balance.cjs`。
