...
- 首页保留循环角色动作、原图集数字和音乐音量滑块；动画与装备绘制继续使用现有高清资源。位图数字的字形宽度保持不变、只收紧了字距（推进 = 字形宽 × 0.86），主页面的 `30`、`100/180`、`373/1000`、`19980` 比原来紧凑。
...
- **排行榜**：消息页新增一页，按等级／金杯／天梯积分排序，每周固定种子生成 14 名离线松鼠并把玩家插进去，同周内榜单稳定，页面明确标注为离线模拟。**天梯赛 30 级才开启**：不够级时自己的金杯与积分不显示（破折号 + 未参赛），看金杯／积分榜时对手一律按 30 级起算。
...
- **每日任务**：首页「活动」里除了每日礼包，还有每天 4 条随机任务。任务池 8 类（胜场／战斗场次／关卡通关／竞技场／抽奖／装备合成／武技升级／消耗体力），**进度取自当天真实行为计数**，同一天刷新页面不会重抽，0 点随每日统计一起重置。奖励为**金松果 10~30 + 经验 50~80**，再从技能／武器卷轴（3~5）、大小体力药剂、普通／超级经验丸、挑战书、天使果实种子里抽 1~3 种，界面上每一项都带图标和名字。当礼包或任一任务可领时，首页「活动」图标会缓慢放大缩小并带一层呼吸光晕（`prefers-reduced-motion` 下自动关闭）。
...
关卡10级开放，三个高手各六关、依次解锁；每轮连续三战，入场一本挑战书，最多复活两次、每次另用一本。中断可继续已付费进度。整轮经验和25金松果在通关时发放；**每次击败对手都可能掉碎片**（不必等到整轮结束），掉率 **59%~79%**（随星级提高）、数量 **2~5**，主类别按螳螂白/仙鹤绿/熊猫蓝，星数越高越可能越级（★3–4越1级、★5–6越2级，蓝碎片封顶），集中在 `GData.STAGE_FRAGMENT`。关卡经验与金松果保持原表数值。**关卡整体难度低于原表**：血量×0.85、力量×0.88、敏捷/速度×0.96（`GData.STAGE_DIFFICULTY`，系数设回 1 即恢复原版强度）；螳螂的「疾风镰刀舞」触发血线从 35% 收紧到 **20%**、每击伤害从 0.55 倍降到 0.36 倍，低血爆发从 1.5 倍降到 1.3 倍，仙鹤开场重击从 1.6 倍降到 1.35 倍。

主动挑战的经验**只看等级差，不再看对手等级的绝对值**：基准 = `27 + 自己等级×1.2`，再乘 `1 + (对手等级−自己等级)×0.14`（下限 0.3 倍、上限 2.2 倍），不设硬上限。**20 级是反超点**：20 级同级一场 51 点，单位体力效率 5.1 首次超过经验竞技场的 150/30 = 5.0，之后一路拉开（60 级 9.9）；单场经验在实际会遇到的等级差下最高 141，仍低于竞技场冠军 150。每升一级所需的挑战场次单调递增（0.7 → 21.8 → 41.4 → 59.5 → 83.7 → 98.8），越到后面越难。

体力药剂**可以顶到自然上限之上**，最高 **999** 点；超出部分不会自然回复，但照样能用来挑战，存档往返也不会被清掉。道具详情里可以「使用 ×10」「全部使用（n）」批量喝药，主界面体力条在超上限时会加金色高亮。
...
本轮确认：状态27/27、数值17/17、战斗规则25/25、拾取11/11、Main集成11/11、扩展玩法34/34、融合9/9、关卡15/15，战斗动画回归通过。最终浏览器自检21/21通过，控制台无错误；真实点击验证了限购、属性书、拾取、跳过汇总、录像不重复结算、跳过战斗后的正常结算与页面归属，以及天使种子（已满仍可合成）、经验竞技场的经验丸加成文案、关卡碎片掉落弹窗、排行榜天梯门槛、超级松鼠、每日任务与领奖、融合材料选择、天梯碎片合成转化丸、升级礼包面板。测试通过只说明对应检查项成立，不等于全部玩法组合或视觉细节已与原版一致；证据与限制见 [还原记录](tools/restoration-2026-09-23.md) 与 [APK 对齐改动表](tools/apk-alignment.md)。
