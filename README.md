...
```bash
node serve.js 8080      # 加 --no-save 可关掉存档文件写入
```
...
> 目录约定：**运行必需的东西全在仓库根**（`index.html`、`css/`、`js/`、`images/`、`audio/`、`serve.js`、`serve.py`、`启动游戏.cmd`、`start-game.ps1`、`启动游戏.command`），
> **开发与验证工具在 `tools/`**（测试、关卡平衡实测、APK 取证、启动器、检查页、规范截图、全部文档），
...
| 手动起服务 | `node serve.js 8080`（没装 Node 就用 `python3 serve.py 8080`） | 游戏目录 `save/progress.json` | 普通浏览器标签 |
...
```bash
node tools/sync/sync.js doctor win     # win 是 peers 里的名字，也可以直接写对端 IP
```
...
把下面这些**拷到任何地方就能玩**（`tools/` 与 `references/` 一个都不需要）：

```
index.html            入口
css/  js/  images/  audio/    前端与素材（游戏里全是相对路径）
...
* **没有轻壳** → 同一份文件里放好 `serve.js`（或 `serve.py`）+ Node（或 Python 3），双击就退回「应用窗口」模式；
* 实测：把上面这套（57.7 MB，含轻壳）单独拷到空目录，`启动游戏.cmd`、`启动游戏.cmd --browser`、`bash 启动游戏.command`、`python serve.py` 四条路都能起、存档都落在该目录的 `save/progress.json`。

`tools/` 里全是**开发与验证**用的东西（测试、实测、取证、检查页、文档），**删掉不影响运行**；
但它**曾经**是双击入口的一部分（`启动游戏.cmd → tools\launchers\start-win.ps1 → tools\serve.js`），
所以早先手工把 `tools/` 移走时会「打不开游戏」——现在这三个文件已经搬到仓库根，`tools/` 可以随便删。
...
| 需要 | 干什么用 | 怎么装 |
| --- | --- | --- |
| **Node.js ≥ 18**（推荐） | `serve.js` 起本地服务器，存档写进 `save/progress.json` | https://nodejs.org 装 LTS 即可 |
| **Python 3**（兜底） | 没有 Node 时用 `serve.py`，功能相同（自带 `/__save`） | 系统自带或 python.org，只用标准库 |
