...
- **系统页新增「画面设置」**：**分辨率**六档（自动／1170×690／1280×720／1440×810／1600×900／1920×1080，按设计尺寸等比缩放并居中，窗口装不下时自动缩小，不会溢出）与**全面屏**开关（两轴铺满窗口、不留黑边，同时请求系统全屏，Esc 可退出）。**音乐音量、静音、分辨率、全面屏都写进存档**（`S.settings`，旧档缺字段会补默认值、坏值回落），所以换浏览器或双机同步后设置照旧；系统页不再放「游戏帮助」「每日礼包」按钮（礼包在「活动」里领），底部那句「跨设备同步」的说明也去掉了。**帮助改成正常页面**（系统分组的「帮助」标签页，双栏排版 + 物品图鉴，不再是挡住主界面的弹窗）。**村庄页六个选项统一金色**（原来黄绿混着填）。**「装备融合」去掉了闪光特效**（预览格的旋转高光、主按钮的呼吸光、状态页入口的流光都不再播放）。**好友页脚只留放大的「师徒」按钮**（去掉「随机挑战」）。**首页「活动」图标的提示动效会跟着可领取状态走**：领完礼包/任务后回到主界面立刻停下，不会一直缩放。
...
- **每日任务**：首页「活动」里除了每日礼包，还有每天 4 条随机任务。**任务池 16 类**（`State.QUEST_TYPES`）：胜场／战斗场次／随机挑战／关卡通关／竞技场／天梯赛／好友切磋／幸运抽奖／合成或融合装备／合成宝石／武技升级／使用道具／商店购买／卖出道具／拾取战斗掉落／消耗体力，**每天用日期做种子从 16 类里抽 4 条**（同一天刷新页面不会换题）。**只抽「当前等级做得完」的任务**：关卡与装备合成 10 级、经验竞技场 11 级、天梯赛 30 级、宝石合成 45 级、武技升级 2 级才有产出（`QUEST_TYPES[].minLevel`），所以 1 级只会看到随机挑战／战斗／抽奖／购买／使用这类任务；升级后旧题里做不了的会被换成当前等级的题，**已经领过的条数继续占位**（不会因为换题多领一份奖励），同一天内仍然做得完的题不会被换掉。**进度取自当天真实行为计数**（`S.dailyCounters`，键跟着任务池自动生成；合成装备、装备融合、宝石合成、使用/购买/卖出道具、拾取掉落、天梯/切磋/随机挑战都会各自计数），0 点随每日统计一起重置。奖励是**纯随机 1~2 项**：金松果（10~30）、经验（50~80）和各种道具放在同一个加权池里抽（`State.QUEST_GOLD` / `QUEST_EXP` / `QUEST_EXTRA_POOL`），**抽到才有**——可能只给一样，也可能给「经验 + 经验丸」这种组合；道具含技能／武器卷轴 3~5、挑战书 1~2、小体力药剂 1~2、天使果实种子、大体力药剂、经验丸。**药丸一次只给 1 个**，高级药丸（超级经验丸）权重只有 0.06，**概率约 1%**。当礼包或任一任务可领时，首页「活动」图标会缓慢放大缩小并带一层呼吸光晕（`prefers-reduced-motion` 下自动关闭），**领完后立刻停**。
...
不同浏览器、地址、端口拥有独立存档。**音乐音量、静音、分辨率、全面屏现在都存在存档里**（`S.settings`，跟着 `save/progress.json` 与双机同步走；`ssdz_music_muted` / `ssdz_music_volume` 两个 localStorage 键只在存档还没载入时给标题画面兜底）。**BGM 在页面打开时就尝试播放**：`boot()` 一开始与标题画面各播一次，不必等素材加载完或点「开始游戏」。浏览器若按自动播放策略拒绝（Chrome/Edge 在没有任何用户手势前不允许带声音播放），页面左下角会出现「♪ 点击画面任意处开启音乐」的提示，并在**第一次点击／按键**时按当前场景补播（战斗里补的是战斗曲），成功即收起提示、不再重试——也就是说在标题画面随便点一下就能听到音乐，不必先进入某个界面。若浏览器允许自动播放（或加了 `--autoplay-policy=no-user-gesture-required`），开屏直接就有音乐，也不会出现提示。`tools/research/bgm-check.html` 用包装过的 `Audio` 记录创建与 `play()` 次数，`tools/research/bgm-autoplay-check.cjs` 则在放行的 Chrome 里跑同一页做对照。
...
4. 菜单选 `7) 扫描 ZeroTier 网段，找对端`：它会**先试 `peers` 里写过的地址**，再扫本机 ZeroTier 网段的 `/24`（每台试 0.8 秒，一般 1~2 秒），找到开着同步服务的机器并写进 `peers`。以后就不用再扫了。**ZeroTier 网段一台都没找到时，它会兜底把本机其它网段（局域网 / Tailscale）也扫一遍**——Windows 的 ZeroTier 网卡名是 `ZeroTier One [b9a18a…]`，早期版本只认 `zt`/`feth` 前缀，于是 Windows 上会误判成「没有 ZeroTier 地址」而扫不到对端（现象就是「mac 能连 win，win 却找不到对端」）。已经知道对面地址时可以直接 `node scripts/sync/sync.js push 10.32.170.33`，不用扫。
...
* 如果**要被覆盖的那一份进度更靠前**（比如你正要把 19 级的本机档推给 21 级的对面），面板会先弹一个确认框，把两边的等级/经验/金松果都列出来，确认后才覆盖（覆盖前自动备份）。

几条规矩（都是为了不丢档）：

* **存档：方向由你点的那一下决定，判断新旧看的是「进度」而不是时间。**
  「把存档送过去」= 用本机的覆盖对面，「取回对面存档」= 用对面的覆盖本机 —— 既然方向是你点的，就照做，不再拦。
  唯一的安全垫是：**要被覆盖的那一份如果进度更靠前**（按 `等级 → 经验 → 金松果` 比），游戏面板会先弹一次确认，
  命令行只提醒一句；无论哪种情况，**覆盖前都会把对方旧档备份**到 `save/backup/progress-<时间>.json`（留最近 10 份）。
  为什么不用时间判断：这游戏自动保存极频繁（打开就存、每次操作都存），**谁刚打开过谁的时间就最新**，
  用 `savedAt` 比「谁更新」永远偏向你正在用的这台机器，于是「取回对面存档」永远被自己挡下来。
  `savedAt` 现在只用于显示。**两边内容完全一样时不会重复推**（每次覆盖都会在对端留一份备份，白推只会堆垃圾）。
  拉取成功后游戏里刷新一下页面就能看到新进度。
...
* **文件**：按 **大小 + 修改时间**（`--verify` 再比 sha1）比对，**只传更新的那一侧**，其余原样不动，**从不删除**任何一方的文件；传过去后会把修改时间对齐，所以下次不会再传一遍。`save/`、`.git/`、`node_modules/`、`src-tauri/target/`、`*.log`、`scripts/sync/sync.config.json`、开发截图（`tools/verification/*.png`、`tools/research/*.png|jpg`、`battle-check/`）等永远不参与（清单在 `sync.js` 的 `DEFAULT_IGNORE`，也可以在配置里的 `ignore` 追加，`!` 开头是例外）。
...
| Windows 上第 7 项扫不到 Mac（或只显示 `本机 ZeroTier 网段：（无）`） | 早期版本的网卡名判据只认 `zt`/`feth`，而 Windows 的 ZeroTier 网卡叫 `ZeroTier One [xxxx]`，于是扫描清单为空 | 已修（认 `ZeroTier` 字样 + 兜底扫本机其它网段，`doctor` 第 2 项会打印「自动扫描网段」）。仍不行就直接 `node scripts/sync/sync.js push <对端IP>`，或把它写进 `scripts/sync/sync.config.json` 的 `peers` |
| Windows 上 `zerotier-cli` 报 `authtoken.secret not found or readable` | 非管理员跑 `zerotier-cli listnetworks` 的正常现象（不是错误） | 忽略即可（新版已把这段噪声从输出里去掉）；网卡名兜底路径照样能拿到 ZeroTier 地址 |
...
node tools/test-sync.cjs           # 双机同步：临时目录里起两个「机器」，真传文件与存档（37 项）
