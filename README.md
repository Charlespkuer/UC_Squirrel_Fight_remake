...
* **存档**：`push`/`pull` 之前都比 `save/progress.json` 的时间，**对方更新时不覆盖**，只提示；确实要用本机这份盖掉对面就加 `--force`。**两边时间（差 1 秒内）与大小都一样时不再重复推**——每次覆盖都会在对端留一份备份，白推只会堆垃圾。覆盖前旧档会自动备份到 `save/backup/progress-<时间>.json`（只留最近 10 份）。拉取成功后游戏里刷新一下页面就能看到新进度。
