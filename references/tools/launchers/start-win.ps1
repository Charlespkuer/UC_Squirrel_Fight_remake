...
#
# 这个脚本是 start-win.cmd 的实际执行部分。分工原因：cmd.exe 读 .cmd 里的 UTF-8
...
#
# 用法：启动游戏.cmd [端口] [--no-save] [--browser] [--stop]
...
# ---------- 定位游戏根目录（便携包里脚本就在包根；仓库里在 references/tools/launchers）----------
$here = $PSScriptRoot
...
$serveJs = Join-Path $root 'references\tools\serve.js'
$servePy = Join-Path $root 'references\tools\serve.py'
