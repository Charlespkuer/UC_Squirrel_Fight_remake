 ...
 *
 *   node references/tools/build-tauri-web.cjs          # 生成快照（打安装包前）
 *   node references/tools/build-tauri-web.cjs --check  # 检查快照是不是最新的
 *
 * 生成时写 web/BUILD.txt（内容指纹 + 文件数 + 时间），--check 用它判断是否过期。
 * 打安装包用：node references/tools/build-tauri-app.cjs --installer（它会临时生成、打完自动删掉）。
 */
...
if (CHECK) {
  ...
  if (!stamp) {
    console.error('[x] src-tauri/web 还没有生成过：先跑 node references/tools/build-tauri-web.cjs');
    process.exit(1);
  }
  ...
  if (stale) {
    ...
    console.error('    重新生成：node references/tools/build-tauri-web.cjs');
    process.exit(1);
  }
...
fs.writeFileSync(STAMP, [
  '# 这份快照由 references/tools/build-tauri-web.cjs 生成；桌面版（Tauri）嵌的就是它。',
