 ...
 *
 *   node references/tools/build-tauri-app.cjs            # 编译 → 把 exe 拷到 src-tauri/dist/（约 3 MB）
 *   node references/tools/build-tauri-app.cjs --clean    # 编译 → 拷贝 → cargo clean（删掉 target/，约 2.8 GB）
 *   node references/tools/build-tauri-app.cjs --installer # 另打一个 NSIS 安装包（临时暂存 web/，打完就删）
 *
 * 为什么要有这个脚本：
...
if (!CLEAN) console.log('  想立刻回收编译缓存：node references/tools/build-tauri-app.cjs --clean（会重新编译一次）');
